	...
	def __init__(self,provider, server_url, port, db_name, db_user, db_pass, logger):
		...
		try:
			if "SQL Server" in self.provider:
				self.cnxn = pyodbc.connect('DRIVER={};SERVER={},{};DATABASE={};UID={};PWD={};Encrypt=no;'.format(self.provider,self.server_url, str(self.port), self.db_name, self.db_user, self.db_pass), autocommit = True)
